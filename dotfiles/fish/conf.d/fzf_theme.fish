...
# Initialize background_mode from current OS appearance on each shell start.
# dark-notify only fires on *changes*, so we sync the initial state here.
if defaults read -g AppleInterfaceStyle 2>/dev/null | string match -qi dark
    set -Ux background_mode dark
else
    set -Ux background_mode light
end
